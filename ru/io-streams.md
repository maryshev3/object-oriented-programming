...
Для использования стандартных потоков ввода-вывода необходимо подключить заголовочный файл \<iostream\> директивой препроцессора #include. Подключать \<istream\> или \<ostream\> по отдельности имеет смысл только в том случае, если вам нужны типы потоков, но не нужны объекты cin/cout (например, при объявлении параметров функций).
...
- ***seekg(\<offset\>, \<origin\>)*** - используется для смещения указателя чтения на *\<offset\>* байт, относительно положения *\<origin\>*;

- ***seekp(\<offset\>, \<origin\>)*** - используется для смещения указателя записи на *\<offset\>* байт, относительно положения *\<origin\>*.
...
Например, для смещения указателя чтения на 5 байтов от текущей позиции, необходимо использовать вызов:

*Смещение указателя чтения на 5 байтов относительно текущей позиции*
```cpp
ifstream ifs("example.txt");
...
ifs.seekg(5, ios::cur);
...
std::ostream& operator<<(std::ostream& os, const Person& person) {
    os << "Имя: " << person.m_name << ", Возраст: " << person.m_age;
    return os;
}
